...
CREATE UNIQUE INDEX product_images_product_primary_uq ON public.product_images(product_id, is_primary);
...
CREATE TABLE public.product_videos (
  ...
  duration_seconds int, title jsonb, position int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE public.product_documents (
  ...
  src text NOT NULL, language public.language DEFAULT 'EN',
  size_bytes int, title jsonb, position int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE public.specification_groups (
  id text PRIMARY KEY DEFAULT gen_random_uuid()::text,
  product_id text NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
  key text NOT NULL, label jsonb NOT NULL, position int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TRIGGER specification_groups_set_updated_at BEFORE UPDATE ON public.specification_groups FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TRIGGER specification_items_set_updated_at BEFORE UPDATE ON public.specification_items FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TRIGGER product_seo_set_updated_at BEFORE UPDATE ON public.product_seo FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TABLE public.company_profile (
  ...
  name jsonb NOT NULL,
  mission jsonb, vision jsonb, history jsonb,
